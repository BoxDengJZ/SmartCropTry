...
public class SECropView: UIView {
    ...
    fileprivate func pairPositionsAndViews() {
        if let cornerPositions = cornerLocations {
    ...
    public override func layoutSubviews() {
        ...
        if let imgsize = imageView?.image?.size, let imageBounds = imageView?.frame {
    ...
    override public func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        guard cornerViews.count == 4, touches.count == 1, let first = touches.first else {
            return
        }
